...
class GAN_CLS(object):
    ...
    def build_model(self):
        ...
        self.G = generator.generate(self.z, phi_r)

        #    D_imge_text
        self.D_fake_real = discriminator.discriminate(self.G, phi_r, reuse=False)
        ...
        self.G_loss = -tf.reduce_mean(tf.log(self.D_fake_real + 1e-10))
    ...
    def run(self):
        ...
        for iteration in range(self.niter):
            ...
            if iteration % 200 == 0:
                state = {'G Loss': G_loss, 'D Loss': D_loss, 'Iteration': iteration}
                print(state)
                ...
                samples = self.sess.run(self.G, feed_dict={self.context_r: context, self.z: self.z_sampler(16, self.z_dim)})
    ...
    def generate(self, sentences):
        ...
        return self.sess.run(self.G, feed_dict={self.context_r: context, self.z: self.z_sampler(num_instances, self.z_dim)})
